...
Ajax.handleResponse = function(status, type,url, response, headers){
  ...
  if( status == 402 || status == 401) return Promise.resolve( { accessError: true } );
  else if( status < 500 ) return Promise.resolve( { apiError: true, error: response } );
  else{
    return Promise.resolve( { serverError: true, error: response } );
  }
}
...
Ajax.checkStatus = function(path){
  ...
  .catch(function(err){
    console.log(err)
    if(!headers) headers = {};
    if(/MSIE \d|Trident.*rv:/.test(navigator.userAgent)) return {accessError: true, error: err};
    else return {httpError: true, error: err};
  })
}
...
Ajax.get = function(path, params){
  ...
  .catch(function(err){
    if(!headers) headers = {};
    return {httpError: true, error: err};
  })
}
