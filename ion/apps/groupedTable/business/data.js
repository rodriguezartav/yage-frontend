...
export default function(Business) {
  ...
  Business.prototype.load = function(dataView) {
    ...
    return Ajax.get("/" + this.app.props.route + "/" + dataView)
      .then(function(results) {
        ...
        if (results.length == 0) _this.app.setState({
          content: {}
        });
        ...
        xhr.onreadystatechange = () => {
          if (xhr.readyState === 4) {
            if (xhr.status === 200) {
              ...
              content.photos.list.push({
                url: "https://rodcocr.com/assets/" + fileName
              });
              _this.app.setState({
                content: content
              });
  ...
  Business.prototype.loginWithEmail = function(email) {
    var _this = this;
    return Ajax.post("/login/login/loginWithEmail", {
        email: email
      })
      .then(function(results) {
        if (results.httpError) return Business.handleHttpError(results.error);
  ...
  Business.prototype.loginWithPhone = function(phone) {
    var _this = this;
    return Ajax.post("/login/login/loginWithPhone", {
        phone: phone
      })
      .then(function(results) {
        if (results.httpError) return Business.handleHttpError(results.error);
  ...
  Business.prototype.loginWithCode = function(code) {
    var _this = this;
    return Ajax.post("/login/login/validateCode", {
        code: code
      })
      .then(function(results) {
        if (results.httpError) return Business.handleHttpError(results.error);
