...
class ArrayList extends React.PureComponent {
  ...
  onClick(e){
    ...
    if(!this.state.Renderer && this.props.column.renderer) this.renderExternal(this.props.column.renderer);
  ...
  renderExternal(path){
    var _this = this;
    window.requireLazy(path, function(Renderer){
      _this.setState({renderer: Renderer});
   })
  }
  ...
  renderItem(){
    ...
    if(this.props.column.itemType == "image"){
      return <img src={ item.src || item } />
    }
    else if(this.props.column.renderer){
      var Renderer = this.state.renderer;
      if(Renderer) return <Renderer item={item} />
    }
  ...
  renderItems(){
    ...
    return value.formatValue.map(function(item){
      index++;
      return <span onClick={_this.onClick.bind(_this)} key={index} data-index={index} className="slds-badge">
