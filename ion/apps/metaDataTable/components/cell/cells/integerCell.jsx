...
class IntegerCell extends React.PureComponent {
  ...
  onChange(e){
    ...
    if( int > 0 || int < 0 || int == 0  ) this.setState({value: int });
  }

  renderEdit(){
    var value = this.props.row[this.props.column.name];
    return <input type="number" className="slds-input edit-input" ref="txt_edit_input"
  ...
  renderView(){
    var value = this.props.row[this.props.column.name];
    return <div  className="helper-occupy-width" onClick={this.onClick.bind(this)}>{ value.formatValue }</div>
  }
