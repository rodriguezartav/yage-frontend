...
Columns.getDistintFields = function(rows, groupBy){
  ...
  rows.forEach(function(row){
    ...
    if( row[groupBy] && row[groupBy].formatValue != ""){
      ...
      if( !details[formaValue] ) details[formaValue] = {index: -1, sum: 0, count:0}
      details[formaValue].count++;

      if( groupByColumn.sumOnSort ) details[formaValue].sum += (row[groupByColumn.sumOnSort] || {originalValue:0}).originalValue;
...
Columns.getColumnWidths = function(rows, columns) {
  ...
  var totalWidth = 0;
  Object.keys(columnWidths).forEach(function(width){ totalWidth+= columnWidths[width]; });

  return {columnWidths: columnWidths, totalWidth: totalWidth};
